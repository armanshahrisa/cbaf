...
This function scans all the cancer studies to examine presence of _RNA-seq_, _microRNA-seq_, _microarray_ and _methylation_ data. It requires a character string to name the output result.
```
availableData("name")
```    
...
If the user types yes or there is no excel file with the given name, the function continues with printing a message in the R console to inform the user it is scanning all the available cancer studies. Then, a progress bar will appear in the next line.
...
This function obtaines and stores the supported data for at least one group of genes across multiple subgroups of a cancer study.

It requires at least three variables:

- __genesList__, a list that contains at least one gene group.
