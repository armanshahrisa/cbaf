...
This function scans all the cancer studies to examine presence of _RNA-seq_, _microRNA-seq_, _microarray_ and _methylation_ data. It requires a name for calling the output result and excel file.
```
    availableData("name")
```    

 There is another argument that enables the function to save the result as an excel file. The default value is `excelFile = TRUE`. To stop saving excel file, set `excelFile = FALSE`.
...
```
The progress bar gradually proceeds until it reaches 100%. Until now, 161 cancer studies are registered in cBioPortal. Therefore, the function would requires some time to finish.
...
Upon finishing, the output is accessible with the given name as a variable. if `excelFile = FALSE`, an excel file containing the results is also generated in the present directory. Both variable and excel file contain different columns: cancer_study_id, cancer_study_name, RNA.seq, microRNA.seq, microarray of mRNA, microarray of miRNA, methylation and description.

## obtainOneStudy()

This function obtaines and stores the supported data types for at least one group of genes across multiple subgroups of a cancer study.

It requires at least three variables:
...
Function contains other options that can be used:
