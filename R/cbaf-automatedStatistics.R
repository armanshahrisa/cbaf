...
#' specific cutoff in the selected study / subgroups of the study. Furthermore, it looks for the genes that show the highest values
#' in each study / study subgroup.
#'
#' @details
#' \tabular{lllll}{
...
#'
#' @usage automatedStatistics(obtainedDataType = "multiple studies", submissionName, desiredTechnique,
...
#' \code{"single study"} and \code{"multiple studies"}. The function uses \code{obtainedDataType} and \code{submissionName} to construct
#' the name of input data. Default value is \code{obtainedDataType}.
#'
...
#' \code{frequencyPercentage}, which is the number of samples having the value greather than specific cutoff divided by the total sample
#' size for every study / study subgroup;
#' \code{frequency ratio}, which shows the number of selected samples divided by the total number of samples that give the frequency
#' percentage for every study / study subgroup -to know selecected and total sample sizes only;
#' \code{Mean Value}, that contains mean value of selected samples for each study;
#' \code{Median Value}, which shows the median value of selected samples for each study.
#'
...
#'
#' @param top.genes a logical value that, if set as \code{TRUE}, cause the function to create three data.frame that contain the five
#' top genes for each cancer. To get all the three data.frames, \code{`Frequency.Percentage`}, \code{`Mean.Value`} and
#' \code{`Median`} must have been included for \code{data.presented.as}.
#'
#' @return a list that contains some or all of the following groups, based on what user has chosen: \code{Frequency.Percentage},
#' \code{Top.Genes.of.Frequency.Percentage}, \code{Frequency.Ratio}, \code{Mean.Value}, \code{Top.Genes.of.Mean.Value},
...
#' automatedStatistics(obtainedDataType = "single study", "test", "RNA-seq")
#'
...
automatedStatistics<- function(submissionName, obtainedDataType = "multiple studies", desiredTechnique, calculate = c("frequencyPercentage", "frequencyRatio", "meanValue", "medianValue"),
